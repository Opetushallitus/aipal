...
LEFT JOIN vastaus vastaussukupuoli ON vastaussukupuoli.vastaajaid = vastaaja.vastaajaid
  AND vastaussukupuoli.kysymysid = ((
    SELECT kysymys_1.kysymysid
    FROM kysymys kysymys_1
    WHERE (kysymys_1.kysymysryhmaid IN (
      SELECT kysymysryhma_1.kysymysryhmaid
      FROM kysymysryhma kysymysryhma_1
      WHERE kysymysryhma_1.valtakunnallinen = true
      AND kysymysryhma_1.taustakysymykset = true
      AND kysymysryhma_1.tila = 'julkaistu'
      AND kysymysryhma_1.selite_fi = 'AVOP Taustatiedot'
    ))
    AND kysymys_1.kysymys_fi = 'Sukupuoli'
  ))
LEFT JOIN monivalintavaihtoehto monivalintasukupuoli ON monivalintasukupuoli.kysymysid = vastaussukupuoli.kysymysid
  AND vastaussukupuoli.numerovalinta = monivalintasukupuoli.jarjestys
LEFT JOIN vastaus vastausika ON vastausika.vastaajaid = vastaaja.vastaajaid
  AND vastausika.kysymysid = ((
    SELECT kysymys_1.kysymysid
    FROM kysymys kysymys_1
    WHERE (kysymys_1.kysymysryhmaid IN (
      SELECT kysymysryhma_1.kysymysryhmaid
    ...
    AND kysymys_1.kysymys_fi = 'Ikä'
  ))
LEFT JOIN monivalintavaihtoehto monivalintaika ON monivalintaika.kysymysid = vastausika.kysymysid
  AND vastausika.numerovalinta = monivalintaika.jarjestys
LEFT JOIN vastaus vastauspohjakoulutus ON vastauspohjakoulutus.vastaajaid = vastaaja.vastaajaid
  AND vastauspohjakoulutus.kysymysid = ((
    SELECT kysymys_1.kysymysid
    FROM kysymys kysymys_1
    WHERE (kysymys_1.kysymysryhmaid IN (
      SELECT kysymysryhma_1.kysymysryhmaid
    ...
    AND kysymys_1.kysymys_fi = 'Pohjakoulutus'
  ))
LEFT JOIN monivalintavaihtoehto monivalintapohjakoulutus ON monivalintapohjakoulutus.kysymysid = vastauspohjakoulutus.kysymysid
  AND vastauspohjakoulutus.numerovalinta = monivalintapohjakoulutus.jarjestys
WHERE kysymys.vastaustyyppi <> 'vapaateksti'
AND NOT (vastaus.kysymysid IN (
  SELECT kysymys_1.kysymysid
  FROM kysymys kysymys_1
  WHERE (kysymys_1.kysymysryhmaid IN (
    SELECT kysymysryhma_1.kysymysryhmaid
...
AND (monivalintasukupuoli.teksti_fi IS NOT NULL
  OR monivalintaika.teksti_fi IS NOT NULL
  OR monivalintapohjakoulutus.teksti_fi IS NOT NULL
)
WITH NO DATA;
