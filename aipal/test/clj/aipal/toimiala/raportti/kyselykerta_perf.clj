...
(defn async-http-requ [url uid user-id context callback]
...
(defn kyselekertaid->perf-fn [base-url id userid]
  (let [url (str base-url "/api/raportti/kyselykerta/" id)
        requ-fn (partial async-http-requ url userid)
...
(deftest ^:performance kyselykerta-raportti []
  (let [base-url (or (System/getenv "AIPAL_URL") "http://192.168.50.1:8082")
        ...
        test-reqv (mapv #(kyselekertaid->perf-fn base-url %1 userid) test-ids)]
    (println test-ids)
    (run-simulation
      [{:name "Satunnaistettu kyselykerta raportin suorituskykytesti"
        :requests test-reqv}]
      4 {:root "target/perf-report"})))
